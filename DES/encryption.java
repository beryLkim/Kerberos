...
public class encryption {
	...
	public static int miwen[][]=new int[8][8];
	

public static String Operation(String min,String Key) throws UnsupportedEncodingException{
	int mingwen[][]=new int[8][8];//输入明文
	int miyao[][]=new int[8][7];//密钥
	miyao=BinToStr_Convert.StrKeyToInt(Key);
	String Binary=BinToStr_Convert.StrToBin(min);
	int length=Binary.length();
	int size=(length-1)/64;
	String []str=new String[size+1];
	for(int i=0;i<size;i++){
		str[i]=Binary.substring(i*64,(i+1)*64);
	}
	str[size]=Binary.substring(size*64, length);
	String result="";
	for(int i=0;i<size;i++){
		int[]re=BinToStr_Convert.BinStrToIntArray(str[i]);
		for(int j=0;j<8;j++){
			for(int k=0;k<8;k++){
				mingwen[j][k]=re[j*8+k];
			}
		}
		result=result+Des(mingwen,miyao);
	}
	int[]re2=BinToStr_Convert.BinStrToIntArray(str[size]);
	...
	for(int i=0;i<64-str[size].length();i++){
		int p=i/8;
		int q=i%8;
		mingwen[7-p][7-q]=0;
	}
	...
	result=BinToStr_Convert.BinToHexStr(result);
	//System.out.println(result);
	result=64-str[size].length()+"-"+result;
	return result;
	
}

public static String Des(int M[][],int[][]miyao){
	...
	for(int j=0;j<8;j++){
		for(int k=0;k<8;k++){
			int p1=(IP_[j][k]-1)/8;
			int p2=(IP_[j][k]-1)%8;
			miwen[j][k]=miwen2[p1][p2];
		}
	}
	StringBuffer result=new StringBuffer();
	for(int j=0;j<8;j++){
		for(int k=0;k<8;k++){
			result.append(miwen[j][k]);
		}
	}
...
public static int[][] GetKey(int m,int [][]Key1){//获得密钥
