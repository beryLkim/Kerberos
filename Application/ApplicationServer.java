...
public class ApplicationServer {
	String Kv;
	ServerSocket Server;
	volatile int [][]cpKey=new int[5][5];
	
	private Lock lock=new ReentrantLock();
    ConcurrentMap<String, Socket>ClientList = new ConcurrentHashMap<String, Socket>();//用户名，对应的Socket
 //   ConcurrentMap<String,String>cpKey=new ConcurrentHashMap<String,String>();
		...
		public void run(){
			try {
				...
				while(true)
				{
					...
					case "001001":
					case "111101":	
						if(control.equals("001000"))
						{
							int mark1=Integer.valueOf(msgFromClient.substring(9,10));
							int mark2=Integer.valueOf(msgFromClient.substring(13,14));
							if(cpKey[mark1][mark2]==1)
							{
								break;
							}
							else
					...
					case "001010":
						bw.writeUTF("001011");
						int mark1=Integer.valueOf(msgFromClient.substring(10,11));
						for(int i=1;i<5;i++)
						{
							cpKey[mark1][i]=0;
							cpKey[i][mark1]=0;
