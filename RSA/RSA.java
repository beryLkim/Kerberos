...
public static String Signature(String msg,BigInteger MySk,BigInteger Myn) throws UnsupportedEncodingException{
	String BinStr=Convert.StrToBin(msg);
	BigInteger BigMsg=Convert.BinToBig(BinStr);
	BigInteger Hash=new BigInteger(MD5.MD5(BigMsg.toString()),16);
	BigInteger Sig=En_Decryption.En_Decryption(Hash,MySk,Myn);//签名
	String result=Sig.toString();
	return result;
}

public static boolean Verify(String msg,String Sig,BigInteger Otn) throws UnsupportedEncodingException{
	String BinStr=Convert.StrToBin(msg);
	BigInteger BigMsg=Convert.BinToBig(BinStr);
	BigInteger Hash=new BigInteger(MD5.MD5(BigMsg.toString()),16);
	BigInteger OtPk=new BigInteger("65537");
	if(Hash.equals(En_Decryption.En_Decryption(new BigInteger(Sig), OtPk, Otn))){
		return true;
	}
	else{
		return false;
